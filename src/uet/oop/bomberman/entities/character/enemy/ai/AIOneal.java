...
public class AIOneal {
	...
	public AIOneal(Oneal oneal, Board board)
	{
		_oneal = oneal;
		_board = board;
	...
	public int calculateDirection() {
		int sx = Coordinates.pixelToTile(_oneal.getY() - 1);
		int sy = Coordinates.pixelToTile(_oneal.getX());
		...
		while (!_queueX.isEmpty())
		{
			...
			for(int direction = 0; direction < 4; ++direction)
			{
				...
				if (bomb != null && _oneal.getOnBomb().indexOf(bomb) == -1) continue;
